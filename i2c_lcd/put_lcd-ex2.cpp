...
void setup(){
    ...
    Particle.function("data", lcd_print_base64);
...
int lcd_print_base64(String args){
    char cbuff[MAX_VARIABLE_LENGTH];
    args.getBytes(reinterpret_cast<unsigned char*>(cbuff), MAX_VARIABLE_LENGTH);
    ...
    // base64_encode(data, cbuff, strlen(cbuff));
    unsigned len = base64_decode(data, cbuff, strlen(cbuff));

    unsigned int *raw_data = reinterpret_cast<unsigned int*>(cbuff);
    itoa(*raw_data++, cbuff, 10);
